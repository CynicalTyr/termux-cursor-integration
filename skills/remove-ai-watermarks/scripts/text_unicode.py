...
LATIN_CONFUSABLES: dict[int, str] = {
    ...
    0x0445: "x",
    0x0456: "i",
    **{0xFF21 + i: chr(ord("A") + i) for i in range(26)},
    **{0xFF41 + i: chr(ord("a") + i) for i in range(26)},
}
...
def _is_strip_cp(cp: int) -> bool:
    if cp in STRIP_CODEPOINTS or cp in _VS_SUPPLEMENT:
        return True
...
def _is_emoji_base(cp: int) -> bool:
    if 0x1F000 <= cp <= 0x1FAFF:
        return True
    if 0x2600 <= cp <= 0x27BF or 0x2B00 <= cp <= 0x2BFF:
        return True
...
def inspect_text(
    ...
    *,
    ...
    notes = [
        ...
        "Emoji ZWJ/VS15/VS16 after an emoji base is preserved unless --strip-emoji-glue.",
    ]
    if not hits:
        notes.append("No deterministic Layer A carriers detected.")
